...
function buildUserPrompt(context: CommentContext): string {
	...
	promptParts.push(`- PR #${context.prContext.number}: ${context.prContext.title}`);
	promptParts.push(`- Repository: ${context.prContext.owner}/${context.prContext.repo}`);
	promptParts.push('');
	...
	promptParts.push(`Developer Question from @${context.questionComment.userLogin}:`);
	promptParts.push('```');
	...
	promptParts.push(`2. Reference the original AI review comment and code as needed.`);
	promptParts.push(`3. Provide specific examples or code snippets when helpful.`);
	promptParts.push(`4. If the question is about a technical concept, explain it simply.`);
