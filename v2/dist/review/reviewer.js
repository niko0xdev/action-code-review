...
import { hasTestFileChanges, normalizeCategories, resolveCrossFindingConflicts, sanitizeReplacements, trivialPrFastPath, } from './validation.js';
import { validateFindings } from './validator.js';
export async function runReview(context, harness, options = {}) {
    ...
    for (let start = 0; start < groups.length; start += 3) {
        const outcomes = await Promise.allSettled(groups.slice(start, start + 3).map(async (group) => {
            const groupContext = {
                ...
                diff: {
                    ...scoped.diff,
                    files: scoped.diff.files.filter((f) => group.files.includes(f.filename)),
                },
            };
    ...
    const minimum = options.minSeverity === undefined
        ? SEVERITY_ORDER.low
