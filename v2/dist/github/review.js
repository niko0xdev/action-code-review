...
export function buildReviewPayload(findings, headSha, options = {}) {
    ...
    return {
        commit_id: headSha,
        event: options.blockOnIssues === false || comments.length === 0
            ? 'COMMENT'
            : 'REQUEST_CHANGES',
        comments,
    };
}
export async function publishReview(octokit, params) {
    ...
    if (params.requireWritePermissions && params.actor) {
        ...
        if (!hasWrite) {
            core.warning(`[review] Actor ${params.actor} lacks write permission — skipping APPROVE/REQUEST_CHANGES escalation (review still posted).`);
        }
    }
    const existingIds = await fetchExistingCommentIds(octokit, owner, repo, prNumber);
    const findings = result.findings.filter((finding) => !existingIds.has(normalizeCommentId(finding)));
    ...
    if (params.bufferInlineComments) {
        ...
        if (probe.length > 0) {
            core.warning(`[buffer] ${probe.length} finding(s) filtered as test/probe — NOT posted`);
        }
    ...
    if (findingsToPublish.length > 0) {
        ...
        try {
            ...
            core.warning(`Batch review failed: ${error instanceof Error ? error.message : String(error)}`);
    ...
    try {
        ...
        for (const review of reviews) {
            ...
            for (const comment of comments) {
                const match = comment.body?.match(/<!-- ai-review-id:([a-f0-9]{12}) -->/);
                if (match)
                    ids.add(match[1]);
            }
        }
...
export function buildReplyBody(body, finding) {
    const trimmed = body.trim();
    return finding
        ? `${trimmed}\n\n<!-- ai-review-id:${normalizeCommentId(finding)} -->`
        : trimmed;
}
export async function replyToReviewComment(octokit, params) {
    if (!params.body || !params.body.trim())
        throw new Error('A non-empty reply body is required to reply to a review comment.');
    ...
    if (!target.pull_request_url?.endsWith(expectedUrl) ||
...
export async function postReviewReply(octokit, reply, target) {
 ...
 */
export function renderToolFindingsSection(toolFindings, diagnostics) {
