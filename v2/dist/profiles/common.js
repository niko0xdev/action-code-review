...
function packageDependency(repo, name) {
...
function hasSourceWithExtension(repo, extension) {
    let found = false;
...
const SIGNALS = [
    ...
    },
    {
        ...
        test: (repo) => hasFile(repo, 'build.gradle', 'build.gradle.kts', 'settings.gradle', 'settings.gradle.kts'),
    },
    {
        id: 'kotlin',
...
export function detectProfiles(repositoryPath) {
    const found = new Map();
    for (const signal of SIGNALS) {
        ...
        if (existing)
            ...
            found.set(signal.id, { id: signal.id, evidence: [signal.evidence] });
    }
    return [...found.values()];
}
