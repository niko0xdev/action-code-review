...
/**
 * Harness abstraction (spec §29): one interface, deliberately small.
 * PiHarness is the launch implementation; future harnesses plug in here.
 */
...
/** Raw finding shape a harness is asked to produce (subset-validated later). */
...
/**
 * Build the review instruction sent to the harness. Restates the
 * prompt-injection defense from spec §24 and requests strict JSON output.
 */
export function buildReviewPrompt(
	...
	const profileLine = formatProfiles(profiles);
	...
	return [
		...
		'',
		'Changed files:',
		fileLines || '(no text patches available)',
		'',
		'Respond with ONLY this JSON shape (no prose outside JSON):',
		'{"findings": [{"severity": "critical|high|medium|low", "confidence": 0.0-1.0, "category": "correctness|security|regression|error-handling|data-integrity|concurrency|performance|maintainability|testing|compatibility", "path": "file/path", "line": <1-based line in the new version>, "title": "...", "description": "...", "impact": "...", "suggestion": "...", "replacement": "exact replacement code or null"}], "summary": "concise overall review summary", "risk": "critical|high|medium|low|none"}',
	]
		.filter((part) => part !== '')
		.join('\n');
}

function formatProfiles(profiles: DetectedProfile[]): string {
	return profiles.map((p) => p.id).join(', ');
}

/** Parse + leniently validate raw harness output into structured form. */
...
/** Promote parsed harness output into a full ReviewResult. */
