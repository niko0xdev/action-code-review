...
export function applyQualityGate(
	...
	minConfidence: SecurityConfidence = 'medium',
	maxFindings = 20
): QualityGateResult {
	const validated: SecurityFinding[] = [];
	const rejected: SecurityFinding[] = [];
	...
	validated.sort((a, b) => {
		const sevDiff =
			(SEVERITY_RANKS[b.severity] ?? 0) - (SEVERITY_RANKS[a.severity] ?? 0);
		if (sevDiff !== 0) return sevDiff;
		return (
			(CONFIDENCE_RANKS[b.confidence] ?? 0) -
