import { CURATED_SECURITY_SKILLS, type SecuritySkill } from './registry.js';

/**
 * Selects curated cybersecurity skills based on identified risk domains.
 * Spec reference: §7, §14.
 */
export function selectSecuritySkills(domains: string[]): SecuritySkill[] {
	if (!domains || domains.length === 0) {
		// Default to foundational AppSec skills
		return CURATED_SECURITY_SKILLS.filter(
			(s) =>
				s.domain === 'authentication' ||
	...
	const matched = CURATED_SECURITY_SKILLS.filter((s) =>
 ...
 */
export function renderSkillsForPrompt(skills: SecuritySkill[]): string {
