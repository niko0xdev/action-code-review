...
import type { RiskClassification, RiskClassificationLevel } from '../types.js';

interface DomainPattern {
	domain: string;
	pathRegex: RegExp;
	contentRegex?: RegExp;
	weight: number; // 1: low, 2: medium, 3: high, 4: critical
	reason: string;
}

const SECURITY_DOMAIN_PATTERNS: DomainPattern[] = [
	// Auth & Authorization & Access Control (Critical)
	{
		domain: 'authentication',
		pathRegex:
			/(?:auth|login|signin|signup|session|jwt|oauth|sso|token|credential|mfa|2fa)/i,
		contentRegex:
			/(?:jwt\.verify|jwt\.sign|bcrypt|argon2|passport|authenticate|session\.|createSession|verifyPassword)/i,
		weight: 4,
	...
	},
	{
		...
		pathRegex:
			/(?:authz|permission|role|rbac|abac|policy|acl|guard|canAccess|access[_-]?control)/i,
		contentRegex:
			/(?:hasPermission|requireRole|checkPermission|canAccess|isAuthorized|isAllowed|authorize)/i,
		weight: 4,
	...
	// Cryptography & Secrets
	{
		...
		contentRegex:
			/(?:crypto\.createCipher|crypto\.createDecipher|AES|RSA|HMAC|privateKey|publicKey|crypto\.subtle)/i,
		weight: 3,
	...
	// Payment & Billing
	{
		domain: 'payments',
		pathRegex:
			/(?:payment|billing|stripe|checkout|subscription|invoice|wallet|pricing)/i,
		contentRegex:
			/(?:stripe\.charges|stripe\.paymentIntents|refund|charge|creditCard)/i,
		weight: 3,
	...
	// File Uploads & Parsers
	{
		...
		pathRegex:
			/(?:upload|storage|s3|file[_-]?handler|multer|busboy|tar|zip|unzip|extractor)/i,
		contentRegex:
			/(?:multipart|upload\.single|createWriteStream|extract|unzip|fs\.writeFile)/i,
		weight: 3,
	...
	// Serialization & Deserialization
	{
		...
		pathRegex:
			/(?:deserializ|unmarshal|parser|protobuf|msgpack|yaml[_-]?parser)/i,
		contentRegex:
			/(?:eval\(|pickle\.loads|yaml\.load\(|JSON\.parse\(|unserialize)/i,
		weight: 3,
	...
	// Network & HTTP client (SSRF surfaces)
	{
		...
		pathRegex:
			/(?:proxy|gateway|fetcher|http[_-]?client|webhook|redirect|crawler)/i,
		contentRegex: /(?:axios\.|fetch\(|http\.request|needle|got\(|urllib|curl)/i,
		weight: 3,
		reason:
			'Outbound HTTP client or reverse proxy changes (potential SSRF surface)',
	},
	...
	// Shell & Command execution (RCE)
	{
		...
		contentRegex:
			/(?:child_process|exec\(|spawn\(|execSync|system\(|shell_exec|execFile)/i,
		weight: 4,
		reason:
			'Shell / OS process execution changes (potential command injection surface)',
	},
	// CI/CD & GitHub Actions Workflows
	{
		domain: 'cicd-security',
		pathRegex:
			/(?:\.github\/workflows\/.*\.ya?ml|Dockerfile|docker-compose.*\.ya?ml|\.gitlab-ci\.yml|Jenkinsfile)/i,
		contentRegex:
			/(?:pull_request_target|permissions:|GITHUB_TOKEN|secrets\.|eval|bash -c)/i,
		weight: 4,
		reason:
			'CI/CD pipeline, GitHub Action workflow, or container build changes',
	},
	// Infrastructure as Code / Kubernetes / Terraform
	{
		domain: 'cloud-infrastructure',
		pathRegex:
			/(?:terraform|k8s|helm|cloudformation|pulumi|\.tf$|values\.ya?ml)/i,
		weight: 3,
	...
	// Dependencies & Lockfiles (Supply Chain)
	{
		domain: 'supply-chain',
		pathRegex:
			/(?:package\.json|package-lock\.json|pnpm-lock\.yaml|yarn\.lock|requirements\.txt|Pipfile|go\.mod|Cargo\.toml|pom\.xml|build\.gradle)/i,
		weight: 2,
	...
	// AI / LLM / Agent permissions / Tool execution
	{
		...
		contentRegex:
			/(?:systemPrompt|temperature|tool_choice|executeTool|callTool|modelProvider)/i,
		weight: 3,
 ...
 */
export function classifyPrRisk(
	...
	let level: RiskClassificationLevel = 'low';
	if (maxWeight >= 4) {
