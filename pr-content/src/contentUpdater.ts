...
export async function updatePullRequestContent(
	...
	try {
		...
		// Attribution stays in a hidden HTML comment (not visible Markdown)
		// so reviewers see only the description, while the model that
		// produced it is still recoverable from the raw PR body.
		const safe = (v: string) => v.split('--').join('- -').replace(/[\r\n]/g, ' ');
		const footer = `<!-- Auto-generated by AI Code Review · model: ${safe(model ?? 'gpt-4')} -->`;
		const finalBody = `${finalDescription}\n\n${footer}`;
