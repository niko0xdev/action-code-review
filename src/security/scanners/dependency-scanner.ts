 ...
 */
export function scanDependenciesInDiff(
	...
	for (const file of changedFiles) {
		...
		if (!isPackageJson || !file.patch) continue;
		...
		for (const rawLine of lines) {
			...
			if (rawLine.startsWith('+') && !rawLine.startsWith('+++')) {
				...
				if (
					/"(preinstall|postinstall|install)"\s*:\s*"(?:curl|wget|bash|sh|node -e|eval)/i.test(
						addedContent
					)
				) {
					findings.push({
						...
						evidence: [
							{
								type: 'scanner',
								description:
									'Package manifest includes a lifecycle script executing remote or shell commands on install.',
								file: file.filename,
								line: currentLine,
							},
						],
						...
						remediation:
							'Avoid lifecycle install scripts that fetch or execute arbitrary remote scripts during installation.',
				...
				if (/":\s*"(\*|latest)"/i.test(addedContent)) {
					findings.push({
						...
						evidence: [
							{
								type: 'scanner',
								description:
									'Dependency pinned to "*" or "latest", allowing arbitrary upstream updates without lock verification.',
								file: file.filename,
								line: currentLine,
							},
						],
						...
						remediation:
							'Pin specific semver ranges (e.g. ^1.2.3 or exact 1.2.3) and commit package lockfiles.',
						scannerSources: ['dependency-scanner'],
					});
				}
