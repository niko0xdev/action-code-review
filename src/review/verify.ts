 ...
 */
const VERIFY_TARGET_SEVERITIES: Severity[] = ['critical', 'high'];
...
export function buildVerifyPrompt(
	...
	const toolSection = toolFindings.length
		? `\nStatic analyzer evidence (from prelint):\n${toolFindings
				...
				.map(
					(f) =>
						`- [${f.tool}/${f.code}] ${f.path}:${f.line} (${f.severity}) ${f.message}`
				)
				.join('\n')}\n`
		: '';
...
export async function runVerifyPass(
	...
	const highCritical = options.findings.filter((f) =>
		VERIFY_TARGET_SEVERITIES.includes(f.severity)
	);
	...
	const surviving = options.findings.filter((f) => {
		if (!VERIFY_TARGET_SEVERITIES.includes(f.severity)) return true;
 ...
 */
function identifyFinding(finding: Finding): string {
...
interface VerifyOutput {
	findings: Array<Partial<Finding> & { verified?: boolean }>;
}
